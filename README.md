...
* 修改内容:
   ...
   7. 「功能性」增加混合模式、融合模式与推荐模式，用户可自选不同的过滤等级，在完全纯净的热门流和个性化之间兼顾。
* 开发计划
  - [x] 增加「混合模式」：交替请求热门流和个性化流，既有基于个性化推荐的内容保留舒适区，也有基于热门推荐的内容打破信息茧房。
  - [x] 增加「融合模式」：同时请求探索流和个性化流，并按用户设置比例合并首页内容。
  - [x] 增加「探索模式」：每次刷新首页，随机一个新的起始种子，首页内容几乎没变化的问题。
  - [x] 增加「重置设备指纹」：在探索/混合/融合模式下强制清除并下发全新设备特征码。
...
* **兼顾热门与个性化**：可切换多种过滤等级，纯净模式、探索模式、混合模式、融合模式与个性模式覆盖不同干预强度和合并策略。
...
2. 点击扩展图标打开精美的设置面板：
   ...
   * **融合模式**：扩展同时请求探索内容与个性化内容，并按照设置页中的比例合并首页列表。
   * **个性模式**：扩展不做干预，首页推荐遵循 B 站自身算法。
3. **智能刷新**：在切换开关时，如果扩展检测到你当前正在浏览 B 站页面，会自动帮你重载页面以让新模式立即生效。
4. **刷新设备指纹**：探索模式、混合模式与融合模式下，可点击右下角「刷新设备指纹」按钮，强制更新设备指纹。
---
...
- 「探索模式」：扩展会过滤 cookies 只返回必要的`buvid3` 和 `buvid4`参数，目前 B 站没有做基于 Cookie 的防火墙，也未做参数交叉动态校验，所以**目前没有任何问题**，但不排除未来哪一天 B 站突然丧心病狂地升级了安全策略的可能性。
- 「混合模式」：扩展会交替请求探索流和个性化流，部分请求仍会走 B 站原生 Cookie。
- 「融合模式」：扩展会同时请求探索流和个性化流，再按设置比例合并；其中探索分支只携带必要设备指纹，个性分支保留 B 站原生 Cookie。
- 「个性模式」：扩展不做任何干预，纯原生状态。


#### Q4：在探索模式/混合模式/融合模式下，「刷新设备指纹」按钮是干什么的？
...
**为了提高用户体验**。 因为 B 站前端有 Wbi 动态签名加密（`w_rid` 参数）。在探索模式/混合/融合模式下，如果我们只使用 DNR 在网络层去拦截并强制修改 URL 参数（比如加随机数来防缓存），会直接破坏哈希校验，导致 B 站后端返回 `-403 校验失败`。 **解法**：经逆向发现，B 站的 Wbi 签名只对 URL 的 Query 参数进行哈希验真，并不校验 HTTP 请求头中的 Cookie。因此，扩展利用前端脚本重写 fetch，在请求真正发出的前执行异步阻塞，通知后台 DNR 将规则链（remove/set）编译妥当，随后透传原生 URL，不着痕迹地完成无损清洗个人痕迹。
